 ...
 * @version 0.982
 *
 * @history 0.982 2014-01-08 Added updateHash to retain values of sorting and filtering
...
var Table = (function(){
	...
	*/
	...
	/**
	* Add filter to hash in url so that on reload the sorting order is retained
	*/
	table.store = function(tdata) {
		location.hash = "";
		if (tdata.col)
			location.hash = "sort/col" + tdata.col + "/desc/" + tdata.desc;
		if (tdata.filters) {
			location.hash += "/filter";
			for (var j in tdata.filters) {
				location.hash += "/col" + j + "/" + tdata.filters[j];
			}
		}
	}

	/**
	* Scan the URL for settings and apply sort and/or filter
	*/
	table.restore = function(t) {
		var col;
		var desc;
		var filters;
		var i;
		...
		// find sort settings
		var hasSort = hash.match(/^#sort/)?true:false;
		if (hasSort) {
			col = hash.match(/^#sort\/col[0-9]+/).toString().slice(-1);
			desc = hash.match(/^#sort\/col[0-9]+\/desc\/true/)?true:false;
			// apply sort settings
			table.tabledata[t.id].col = col;
			table.tabledata[t.id].desc = desc;
			// now sort
			table.autosort(t,{'col':col});
		}


		// find filter settings
		var hasFilter = hash.match(/\/filter\//)?true:false;
		if (hasFilter) {
			var name="";
			var value="";

			var filterString = hash.match(/\/filter\/.*/)[0];
			filters = filterString.match(/col[0-9]+\/[^\/]+\b/g);
			for (var i in filters) {
				if (filters.hasOwnProperty(i)) {
				f = filters[i];
				name = f.split("/")[0].replace("col","");
				value = f.split("/")[1];
				// apply filter settings
				var tdata = table.tabledata[t.id];
				if (!tdata.filters)
					tdata.filters = {};
				tdata.filters[+name] = value;
				}
			}
			table.filter(t,t);
		}
	...
	table.auto = function(args) {
		...
		if (tables!=null) {
			for (var i=0,L=tables.length; i<L; i++) {
				...
				// check the hash in url for previous sorting and filtering settings
				table.restore(t);
